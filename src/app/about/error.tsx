...
export default function AboutError({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => { console.error("[About Error]", error); }, [error]);
  return (
    <main className="min-h-screen bg-[#0A0F1E] pt-32 flex items-center justify-center px-6">
      <div className="text-center max-w-lg">
        <div className="text-6xl mb-6">🏢</div>
        <h2 className="text-2xl font-bold text-white mb-3">Could not load page</h2>
        <p className="text-gray-400 mb-8">The about page failed to load. Please try again.</p>
